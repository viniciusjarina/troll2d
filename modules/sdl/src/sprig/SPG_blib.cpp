...
extern void _HLine(SDL_Surface *Surface, Sint16 x1, Sint16 y, Sint16 x2, Uint32 Color);
extern void _HLineAlpha(SDL_Surface *Surface, Sint16 x1, Sint16 y, Sint16 x2, Uint32 Color, Uint8 alpha);
...
void SPG_FadedLine(SDL_Surface *dest,Sint16 x1,Sint16 x2,Sint16 y,Uint8 r1,Uint8 g1,Uint8 b1,Uint8 r2,Uint8 g2,Uint8 b2)
{
	if ( SDL_MUSTLOCK(dest) && _SPG_lock )
		...
		{
		    if(_spg_useerrors)
                SPG_Error("SPG_FadedLine could not lock surface");
			return;
		}
...
void SPG_LineTex(SDL_Surface *dest,Sint16 x1,Sint16 x2,Sint16 y,SDL_Surface *source,Sint16 sx1,Sint16 sy1,Sint16 sx2,Sint16 sy2)
{
	if ( SDL_MUSTLOCK(dest) && _SPG_lock )
		...
		{
		    if(_spg_useerrors)
                SPG_Error("SPG_LineTex could not lock dest surface");
			return;
		}
	if ( SDL_MUSTLOCK(source) && _SPG_lock )
		...
		{
		    if(_spg_useerrors)
                SPG_Error("SPG_LineTex could not lock source surface");
			return;
		}
...
void SPG_Trigon(SDL_Surface *dest,Sint16 x1,Sint16 y1,Sint16 x2,Sint16 y2,Sint16 x3,Sint16 y3,Uint32 color)
{
	...
	if(SPG_GetAA())
	{
        ...
        _AALineAlpha(dest,x1,y1,x3,y3,color, SDL_ALPHA_OPAQUE);
        _AALineAlpha(dest,x3,y3,x2,y2,color, SDL_ALPHA_OPAQUE);
	}
	else
	{
...
void SPG_TrigonFilled(SDL_Surface *dest,Sint16 x1,Sint16 y1,Sint16 x2,Sint16 y2,Sint16 x3,Sint16 y3,Uint32 color)
{
	...
	if(SPG_GetAA())
	{
        ...
        // Draw AA lines
	    _AALineAlpha(dest,x1,y1,x2,y2,color, SDL_ALPHA_OPAQUE);
	    _AALineAlpha(dest,x2,y2,x3,y3,color, SDL_ALPHA_OPAQUE);
	    _AALineAlpha(dest,x3,y3,x1,y1,color, SDL_ALPHA_OPAQUE);
	...
	*/
    ...
    if( y1==y2 )
        _HLine(dest, x1, y1, x2, color);
    else{
        m1 = (Sint32)((x2 - x1)<<16)/(Sint32)(y2 - y1);

        for ( y = y1; y <= y2; y++) {
            _HLine(dest, xa>>16, y, xb>>16, color);
    ...
    if( y2==y3 )
        _HLine(dest, x2, y2, x3, color);
    else{
        m3 = (Sint32)((x3 - x2)<<16)/(Sint32)(y3 - y2);

        for ( y = y2+1; y <= y3; y++) {
            _HLine(dest, xb>>16, y, xc>>16, color);
...
void SPG_TrigonFilledBlend(SDL_Surface *dest,Sint16 x1,Sint16 y1,Sint16 x2,Sint16 y2,Sint16 x3,Sint16 y3,Uint32 color, Uint8 alpha)
{
	...
	if(SPG_GetAA())
	{
        ...
        // Draw AA lines
	    _AALineAlpha(dest,x1,y1,x2,y2,color, alpha);
	    _AALineAlpha(dest,x2,y2,x3,y3,color, alpha);
	    _AALineAlpha(dest,x3,y3,x1,y1,color, alpha);
	...
	/* Upper half of the triangle */
	if( y1==y2 )
		_HLineAlpha(dest, x1, y1, x2, color, alpha);
	else{
		m1 = (Sint32)((x2 - x1)<<16)/(Sint32)(y2 - y1);

		for ( y = y1; y <= y2; y++) {
			_HLineAlpha(dest, xa>>16, y, xb>>16, color, alpha);
	...
	/* Lower half of the triangle */
	if( y2==y3 )
		_HLineAlpha(dest, x2, y2, x3, color, alpha);
	else{
		m3 = (Sint32)((x3 - x2)<<16)/(Sint32)(y3 - y2);

		for ( y = y2+1; y <= y3; y++) {
			_HLineAlpha(dest, xb>>16, y, xc>>16, color, alpha);
...
void SPG_Polygon(SDL_Surface *dest, Uint16 n, Sint16* x, Sint16* y, Uint32 color)
{
    if(n < 3)
    {
        if(_spg_useerrors)
	...
	if(SPG_GetAA())
	{
	    for(i = 0; i < n; i++)
	    {
            _AALineAlpha(dest,ox,oy,x[i],y[i],color, SDL_ALPHA_OPAQUE);
            ox = x[i];
            oy = y[i];
	    }
	}
	else
	{
	    for(i = 0; i < n; i++)
	    {
            _Line(dest,ox,oy,x[i],y[i],color);
            ox = x[i];
            oy = y[i];
	    }
	}
...
void SPG_PolygonBlend(SDL_Surface *dest, Uint16 n, Sint16* x, Sint16* y, Uint32 color, Uint8 alpha)
{
    if(n < 3)
    {
        if(_spg_useerrors)
	...
	if(SPG_GetAA())
	{
	    for(i = 0; i < n; i++)
	    {
            _AALineAlpha(dest,ox,oy,x[i],y[i],color, alpha);
            ox = x[i];
            oy = y[i];
	    }
	}
	else
	{
	    ...
	    for(i = 0; i < n; i++)
	    {
            SPG_LineFn(dest, ox, oy, x[i],y[i],color, callback_alpha_hack);
            ox = x[i];
            oy = y[i];
	    }
	}
...
void SPG_PolygonFilledBlend(SDL_Surface *dest, Uint16 n, Sint16 *x, Sint16 *y, Uint32 color, Uint8 alpha)
{
	if(n<3)
	{
	    if(_spg_useerrors)
	...
	for( sy = ymin; sy <= ymax; sy++){
		...
		for( li = list; li; li = li->next ){
			...
			if( x1 != NULL_POSITION  &&  x2 !=NULL_POSITION ){
				...
				if( alpha == SDL_ALPHA_OPAQUE )
					_HLine(dest, x1, sy, x2, color);
				else
					_HLineAlpha(dest, x1-1, sy, x2, color, alpha);
...
void _PolygonFilledAA(SDL_Surface *dest, Uint16 n, Sint16 *x, Sint16 *y, Uint32 color)
{
	if(n<3)
	{
	    if(_spg_useerrors)
            SPG_Error("_PolygonFilledAA given n < 3");
		return;
	}
	...
	if (SDL_MUSTLOCK(dest) && _SPG_lock)
		...
		{
		    if(_spg_useerrors)
                SPG_Error("_PolygonFilledAA could not lock surface");
			return;
		}
...
void SPG_PolygonFilled(SDL_Surface *dest, Uint16 n, Sint16 *x, Sint16 *y, Uint32 color)
{
    if(SPG_GetAA())
        _PolygonFilledAA(dest, n, x, y, color);
    else
        SPG_PolygonFilledBlend(dest, n, x, y, color, SDL_ALPHA_OPAQUE);
}
...
void SPG_PolygonFadeBlend(SDL_Surface *dest, Uint16 n, Sint16 *x, Sint16 *y, Uint32* colors, Uint8 alpha)
{
	if(n<3)
	{
	    if(_spg_useerrors)
	...
	for( i = 0; i < n; i++ ){
		...
		// Draw the polygon outline (looks nicer)
		if( alpha == SDL_ALPHA_OPAQUE )
			SPG_LineFadeFn(dest,x1,y1,x2,y2,SDL_MapRGB(dest->format, r1,g1,b1),SDL_MapRGB(dest->format, r2,g2,b2), _SetPixel); // Can't do this with alpha, might overlap with the filling
	}
		...
		for( li = list; li; li = (fpline *)li->next ){
			...
			if( x1 !=NULL_POSITION  &&  x2 !=NULL_POSITION ){
				...
				if( alpha == SDL_ALPHA_OPAQUE )
					...
					_SPG_alpha_hack = alpha;
					SPG_LineFadeFn(dest, x1-1, sy, x2, sy, SDL_MapRGB(dest->format, r1, g1, b1), SDL_MapRGB(dest->format, r2, g2, b2), callback_alpha_hack);
				}
...
//==================================================================================
void _PolygonFadeAA(SDL_Surface *dest, Uint16 n, Sint16 *x, Sint16 *y, Uint32* colors)
{
	if(n<3)
	{
	    if(_spg_useerrors)
            SPG_Error("_PolygonFadeAA given n < 3");
		return;
	}
	...
	if (SDL_MUSTLOCK(dest) && _SPG_lock)
		...
		{
		    if(_spg_useerrors)
                SPG_Error("_PolygonFadeAA could not lock surface");
			return;
		}
...
void SPG_PolygonFade(SDL_Surface *dest, Uint16 n, Sint16 *x, Sint16 *y, Uint32* colors)
{
    if(SPG_GetAA())
        _PolygonFadeAA(dest, n, x, y, colors);
    else
        SPG_PolygonFadeBlend(dest, n, x, y, colors, SDL_ALPHA_OPAQUE);
}
