...
static void round_rect_fill(SDL_Surface *bmp, short x,short y, short w,short h, short r, int color);

int filledRoundRectRGBA(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 w, Sint16 h, Sint16 rad, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	round_rect_fill(dst, x, y, w, h, rad, ((Uint32) r << 24) | ((Uint32) g << 16) | ((Uint32) b << 8) | (Uint32) a);
	return 0;
}

static void round_rect(SDL_Surface *dst, short x,short y, short w,short h, short r, int color);

int roundRectRGBA(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 w, Sint16 h, Sint16 rad, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	round_rect(dst, x, y, w, h, rad, ((Uint32) r << 24) | ((Uint32) g << 16) | ((Uint32) b << 8) | (Uint32) a);
	return 0;
}
...
static void round_rect(SDL_Surface *dst, short x,short y, short w,short h, short r, int color)
{
    ...
    if ((color & 255) == 255) 
	{
		...
		do_arc(dst,x + w - r,y + r,0,64,r,(void*)c,draw_point);
		hlineColor(dst,x + r + 1, x + w - r - 1,y ,color);
		
		do_arc(dst,x + r,y + r,64,128,r,(void*)c,draw_point);
		vlineColor(dst,x ,y + r + 1,y + h - r - 1,color);

		do_arc(dst,x + r,y + h - r,128,192,r,(void*)c,draw_point);
		hlineColor(dst,x + r + 1, x + w - r - 1,y + h,color);

		do_arc(dst,x + w - r,y + h - r,192,255,r,(void*)c,draw_point);
		vlineColor(dst,x + w,y + h - r - 1,y + r + 1,color);
	} 
	else
	{
	/* Using Alpha - blended pixel blits */
		do_arc(dst,x + w - r,y + r,0,64,r,(void*)color,draw_point_with_alpha);
		hlineColor(dst,x + r + 1, x + w - r - 1,y ,color);
		
		do_arc(dst,x + r,y + r,64,128,r,(void*)color,draw_point_with_alpha);
		vlineColor(dst,x ,y + r + 1,y + h - r - 1,color);
		
		do_arc(dst,x + r,y + h - r,128,192,r,(void*)color,draw_point_with_alpha);
		hlineColor(dst,x + r + 1, x + w - r - 1,y + h,color);
		
		do_arc(dst,x + w - r,y + h - r,192,255,r,(void*)color,draw_point_with_alpha);
		vlineColor(dst,x + w,y + h - r - 1,y + r + 1,color);
